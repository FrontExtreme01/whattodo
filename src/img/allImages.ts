// Logos
import logoMain from "@/img/logos/logo-wtd.webp";
import logoHorizontal from "@/img/logos/logo-horizontal.webp";
import separador from "@public/icons/separador.webp";
import IconLastminute from "@public/icons/icon-lastminute.svg";
import IconEarlyBird from "@public/icons/early-bird-icon-discounts.svg"
import Stars5 from "@/img/extras/5-stars-opinions.webp";
import TripAdvisorCenotesAdventuring from "@/img/extras/tripadvisor-recommendatios-for-cenotes-adventuring-cancun.webp";
// Footer
import customerSuport from "@/img/extras/customer-support-in-cancun.webp";
import secureCheckout from "@/img/extras/stripe-security-guarantee.webp";

// Testimonials
import review1 from "@public/testimonials/home/zachariah-collier-chichen-itza-cenote-and-valladolid-all-inclusive-tour-review-150x150.webp";
import review2 from "@public/testimonials/home/faisal-mcpherson-chichen-itza-all-iclusive-tour-review-150x150.webp";
import review3 from "@public/testimonials/home/edie-sykes-opinion-review-of-tripadvisor-ATV-and-zipline-in-cancun-150x150.webp";
import review4 from "@public/testimonials/home/bonnie-griffin-chichen-itza-cenote-and-valladolid-all-inclusive-tour-review-150x150.webp";
import review5 from "@public/testimonials/home/harrison-atkinson-review-chichen-itza-cenote-and-valladolid-all-inclusive-tour-150x150.webp";

// Stickers Home
import StickerTripAdvisor from "@/img/extras/banner-tripadvisor.webp";
import StickerHappyCustomer from "@/img/extras/our-happy-customers.webp";

// Extras Golf
import pga1 from "@/img/extras/golf/robert-trent-jones-golf-course-architects.webp";
import pga2 from "@/img/extras/golf/taylor-made-brand-and-golf-accesories.webp";
import pga3 from "@/img/extras/golf/pga-riviera-maya-scorecard.webp";
import ElTinto1 from "@/img/extras/golf/nick-price-design-logo.webp";
import ElTinto2 from "@/img/extras/golf/top-golf-outfits.webp";
import PM1 from "@/img/extras/golf/greg-norman-golf-course-design-in-cancun.webp";
import PM2 from "@/img/extras/golf/playa-mujeres-golf-club-scorecard.webp";
import MoonPalace1 from "@/img/extras/golf/jack-nicklaus-golf-course-design-logo.webp"

// Extras Nightclubs
import placesCocoBongo from "@/img/extras/cocoBongo-places.webp";

// Slides Home
import HomeES1 from "@/img/slides/home/2025-new-year-discounts-in-cancun.webp";
import HomeES2 from "@/img/slides/home/slide-pirate-ship-cancun-jolly-roger.webp";
import HomeES3 from "@/img/slides/home/isla-mujeres-catamaran-tour-premium-service.webp";
import HomeES4 from "@/img/slides/home/atv-zipline-and-cenote-in-cancun-tours.webp";
import HomeES5 from "@/img/slides/home/city-and-taco-tour-and-tequila-tasting-in-cancun-puerto-morelos.webp";
import HomeES6 from "@/img/slides/home/cancun-airport-vip-lounge-service.webp";

// Slides Restaurant
import Rest1 from "@/img/slides/restaurant/best-reastaurants-in-cancun-with-free-welcome-drink.webp";
import Rest2 from "@/img/slides/restaurant/latina-restaurant.webp";
import Rest3 from "@/img/slides/restaurant/senor-frogs.webp";
import Rest4 from "@/img/slides/restaurant/ilios-restaurant.webp";
import Rest5 from "@/img/slides/restaurant/tora-slide.webp";
import Rest6 from "@/img/slides/restaurant/parole-slide.webp";
import Rest7 from "@/img/slides/restaurant/funky-slide.webp";
import Rest8 from "@/img/slides/restaurant/kultura-slide.webp";

// Slides Combos
import Combo1 from "@/img/slides/combos/atv-jungle-adventure-catamaran-slide.webp";
import Combo2 from "@/img/slides/combos/atv-jungle-adventure-glass-boat-slide.webp";
import Combo3 from "@/img/slides/combos/atv-zipline-cenote-speedboat-slide.webp";
import Combo4 from "@/img/slides/combos/cancun-airport-transfer-mera-vip-lounge-access-slide.webp";
import Combo5 from "@/img/slides/combos/cancun-ultimate-5x1-atv-ziplines-cenote-tequila-reef-snorkel-slide.webp";
import Combo6 from "@/img/slides/combos/speedboat-and-catamaran-full-day-adventure-slide.webp";

// Slides Yachts
import Yacht1 from "@/img/slides/yachts/choose-your-ideal-yacht-in-cancun.webp";
import Yacht2 from "@/img/slides/yachts/enjoy-the-luxury-in-cancun.webp";
import Yacht3 from "@/img/slides/yachts/up-to-20-pax-in-private-yacht.webp";
import Yacht4 from "@/img/slides/yachts/yacht-private-service.webp";

// Slides Party
import Party1 from "@/img/slides/party/nightclubs-in-cancun.webp";
import Party2 from "@/img/slides/party/slide-madness-tour.webp";
import Party3 from "@/img/slides/party/bechclubs-in-cancun.webp";
import Party4 from "@/img/slides/party/the-best-events-in-cancun.webp";

const images = {
    es: {
        HOME: [
            { image: HomeES1, alt: "banner 2025", href: "" },
            { image: HomeES2, alt: "banner promocional", href: "" },
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" },
            { image: HomeES6, alt: "banner promocional", href: ""}
        ],
        RESTAURANT: [
            { image: Rest1, alt: "banner restaurantes", href: "" },
            { image: Rest2, alt: "banner promocional", href: "" },
            { image: Rest3, alt: "banner promocional", href: "" },
            { image: Rest4, alt: "banner promocional", href: "" },
            { image: Rest5, alt: "banner promocional", href: "" },
            { image: Rest6, alt: "banner promocional", href: "" },
            { image: Rest7, alt: "banner promocional", href: "" },
            { image: Rest8, alt: "banner promocional", href: "" }
        ],
        COMBOS: [
            { image: Combo1, alt: "banner promocional", href: "" },
            { image: Combo2, alt: "banner promocional", href: "" },
            { image: Combo3, alt: "banner promocional", href: "" },
            { image: Combo4, alt: "banner promocional", href: "" },
            { image: Combo5, alt: "banner promocional", href: "" },
            { image: Combo6, alt: "banner promocional", href: "" }
        ],
        YACHTS: [
            { image: Yacht1, alt: "banner promocional", href: "" },
            { image: Yacht2, alt: "banner promocional", href: "" },
            { image: Yacht3, alt: "banner promocional", href: "" },
            { image: Yacht4, alt: "banner promocional", href: "" }
        ],
        NIGHTCLUB : [
            { image: Party1, alt: "banner promocional", href: "" },
            { image: Party2, alt: "banner promocional", href: "" },
            { image: Party3, alt: "banner promocional", href: "" },
            { image: Party4, alt: "banner promocional", href: "" }
        ],
        BEACHCLUB: [
            { image: Party3, alt: "banner promocional", href: "" },
            { image: Party1, alt: "banner promocional", href: "" },
            { image: Party4, alt: "banner promocional", href: "" }
        ],
        EVENTS: [
            { image: Party4, alt: "banner promocional", href: "" }
        ],
        CHEAP: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        FAVORITES: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        ADVENTURE: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        COUPLES: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        WATER: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        RUINS: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ]
    },
    en: {
        HOME: [
            { image: HomeES1, alt: "banner 2025", href: "" },
            { image: HomeES2, alt: "banner promocional", href: "" },
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" },
            { image: HomeES6, alt: "banner promocional", href: ""}
        ],
        RESTAURANT: [
            { image: Rest1, alt: "restaurant banner", href: "" },
            { image: Rest2, alt: "promotional banner", href: "" },
            { image: Rest3, alt: "promotional banner", href: "" },
            { image: Rest4, alt: "promotional banner", href: "" },
            { image: Rest5, alt: "promotional banner", href: "" },
            { image: Rest6, alt: "promotional banner", href: "" },
            { image: Rest7, alt: "promotional banner", href: "" },
            { image: Rest8, alt: "promotional banner", href: "" }
        ],
        COMBOS: [
            { image: Combo1, alt: "promotional banner", href: "" },
            { image: Combo2, alt: "promotional banner", href: "" },
            { image: Combo3, alt: "promotional banner", href: "" },
            { image: Combo4, alt: "promotional banner", href: "" },
            { image: Combo5, alt: "promotional banner", href: "" },
            { image: Combo6, alt: "promotional banner", href: "" }
        ],
        YACHTS: [
            { image: Yacht1, alt: "banner promocional", href: "" },
            { image: Yacht2, alt: "banner promocional", href: "" },
            { image: Yacht3, alt: "banner promocional", href: "" },
            { image: Yacht4, alt: "banner promocional", href: "" }
        ],
        NIGHTCLUB : [
            { image: Party1, alt: "banner promocional", href: "" },
            { image: Party2, alt: "banner promocional", href: "" },
            { image: Party3, alt: "banner promocional", href: "" },
            { image: Party4, alt: "banner promocional", href: "" }
        ],
        BEACHCLUB: [
            { image: Party3, alt: "banner promocional", href: "" },
            { image: Party1, alt: "banner promocional", href: "" },
            { image: Party4, alt: "banner promocional", href: "" }
        ],
        EVENTS: [
            { image: Party4, alt: "banner promocional", href: "" }
        ],
        CHEAP: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        FAVORITES: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        ADVENTURE: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        COUPLES: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        WATER: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ],
        RUINS: [
            { image: HomeES3, alt: "banner promocional", href: "" },
            { image: HomeES4, alt: "banner promocional", href: "" },
            { image: HomeES5, alt: "banner promocional", href: "" }
        ]
    }
};

const getSlides = (category: string, lang: string) => {
    const categoryImages = images[lang]?.[category] || images['es'][category];

    if (!categoryImages) {
        return [];
    }

    return categoryImages;
};

export {
    getSlides,
    logoMain,
    logoHorizontal,
    customerSuport,
    secureCheckout,
    StickerTripAdvisor,
    StickerHappyCustomer,
    review1,
    review2,
    review3,
    review4,
    review5,
    separador,
    IconLastminute,
    IconEarlyBird,
    pga1,
    pga2,
    pga3,
    ElTinto1,
    ElTinto2,
    PM1,
    PM2,
    MoonPalace1,
    placesCocoBongo,
    Stars5,
    TripAdvisorCenotesAdventuring
};